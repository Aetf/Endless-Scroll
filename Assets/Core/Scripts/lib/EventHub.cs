 ...
 * 
 * Usage examples:
 *  1. EventCenter.AddListener<PropCollectedEvent>(OnPropCollected);
 *     EventCenter.RaiseEvent(new PropCollectedEvent(prop));
 *  2. EventCenter.AddListenerOnce<AnimFinished>(OnAnimFinished);
 *     EventCenter.RaiseEvent(new AnimFinished());
...
namespace UnlimitedCodeWorks {
    ...
    public static class EventHub {
        ...
        public static EventDelegate AddListener<T>(EventDelegate<T> del) where T : GameEvent {
            return EventHubImpl.Instance.AddListener<T>(del);
        }
        ...
        public static EventHub.EventDelegate AddListenerOnce<T>(EventHub.EventDelegate<T> del) where T : GameEvent {
            return EventHubImpl.Instance.AddListenerOnce<T>(del);
        }
        ...
        public static void ClearListener<T>() {
            EventHubImpl.Instance.ClearListener<T>();
        }
        ...
        public static void RemoveListener<T>(EventHub.EventDelegate del) {
            EventHubImpl.Instance.RemoveListener<T>(del);
        }
        ...
        public static void RaiseEvent<T>(T evt) where T : GameEvent {
            EventHubImpl.Instance.RaiseEvent<T>(evt);
        }
        ...
        public static bool QueueEvent<T>(T evt) where T : GameEvent {
            return EventHubImpl.Instance.QueueEvent<T>(evt);
        }
        ...
        public static void MarkAsPermanent<T>() {
            EventHubImpl.Instance.MarkAsPermanent<T>();
        }
        ...
        public static bool HasListener<T>() {
            return EventHubImpl.Instance.HasListener(typeof(T));
        }
        ...
        public static void PrintEventTable() {
            EventHubImpl.Instance.PrintEventTable();
        }
    }

    public class EventHubImpl : MonoBehaviour {
        ...
        public void PrintEventTable() {
            ...
            foreach (KeyValuePair<System.Type, EventHub.EventDelegate> pair in permanentEventTable) {
                InternalLog(pair.Key + "\t\t" + pair.Value);
            }

            InternalLog("=== Messaging Center EventTable ===");
            foreach (KeyValuePair<System.Type, EventHub.EventDelegate> pair in eventTable) {
                InternalLog(pair.Key + "\t\t" + pair.Value);
            }
        ...
        #region Add listener
        public EventHub.EventDelegate AddListener<T>(EventHub.EventDelegate<T> del) where T : GameEvent {
            return AddDelegate(del);
        }

        public EventHub.EventDelegate AddListenerOnce<T>(EventHub.EventDelegate<T> del) where T : GameEvent {
            return AddDelegate(del, true);
        }

        private EventHub.EventDelegate AddDelegate<T>(EventHub.EventDelegate<T> del, bool oneShot = false)
            where T : GameEvent {
        ...
        OnListenerAdding(evtType);
            ...
            EventHub.EventDelegate newDel = null;
            if (oneShot) {
                newDel = e => {
                    del((T) e);
            ...
            EventHub.EventDelegate tempDel;
        ...
        public void RemoveListener<T>(EventHub.EventDelegate del) {
            System.Type evtType = typeof(T);
        ...
        // Main event table
        private Dictionary<System.Type, EventHub.EventDelegate> eventTable = new Dictionary<System.Type, EventHub.EventDelegate>();
        // Event handlers that should never be removed, regardless of calling Cleanup
        private Dictionary<System.Type, EventHub.EventDelegate> permanentEventTable = new Dictionary<System.Type, EventHub.EventDelegate>();
        ...
        if (!HasListener(evtType)) {
            throw new EventHub.RaisingEventException(string.Format("Raising event {0} but no listeners found."
                + " Try marking the listener with EventCenter.MarkAsPermanent.", evtType.Name));
        }
#endif
        }
...
#if LOG_ENABLED
        Debug.Log(string.Format("EventCenter\t{0}\t\t\t{1}", System.DateTime.Now.ToString("hh:mm:ss.fff"), msg));
#endif
        }
        #endregion

        #region Singleton
        private static EventHubImpl instance = null;
        public static EventHubImpl Instance {
            get {
                if (!instance) {
                    instance = GameObject.FindObjectOfType(typeof(EventHubImpl)) as EventHubImpl;
                    // Automatically create the EventCenter if none was found upon start of the game.
                    if (!instance)
                        instance = (new GameObject("EventCenter")).AddComponent<EventHubImpl>();
                }
                return instance;
            }
        }
